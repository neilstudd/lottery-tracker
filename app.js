...
function renderApp() {
    ...
    if (upcomingDrawNums.length) {
        ...
        upcomingTbody.innerHTML = upcomingDrawNums.map(drawNum => {
            ...
            return `<tr><td>${drawNum}</td><td>${formatDate(calculateDrawDate(drawNum))}</td><td><div class="ball-container">${numbers.map(n => renderBallHTML(n, numbers)).join('')}</div></td><td><button class="delete-btn" onclick="deleteTicket(${drawNum})" title="Delete Ticket">&#128465;</button></td></tr>`;
        }).join('');
