...
function renderApp() {
    ...
    if (upcomingDrawNums.length) {
        ...
        upcomingTbody.innerHTML = upcomingDrawNums.map(drawNum => {
            ...
            return `<tr><td>${drawNum}</td><td>${formatDate(calculateDrawDate(drawNum))}</td><td><div class="ball-container">${numbers.map(n => renderBallHTML(n, [])).join('')}</div></td><td><button class="delete-btn" onclick="deleteTicket(${drawNum})" title="Delete Ticket">&#128465;</button></td></tr>`;
        }).join('');
    ...
    document.getElementById('pastTableBody').innerHTML = getCurrentPageDraws().map(draw => {
        ...
        return `<tr><td>${draw.draw_number}</td><td>${formatDate(draw.draw_date)}</td><td><div class="draw-stacked-container"><div class="draw-row"><span class="draw-label">${hasDraw2 ? 'Draw 1:' : ''}</span><div class="ball-container">${draw.draw1.numbers.map(n => renderBallHTML(n, ticketNums)).join('')}${draw.draw1.bonus !== undefined ? `<div class="divider"></div>${renderBallHTML(draw.draw1.bonus, ticketNums, true)}` : ''}</div></div>${hasDraw2 ? `<div class="draw-row"><span class="draw-label">Draw 2:</span><div class="ball-container">${draw.draw2.numbers.map(n => renderBallHTML(n, ticketNums)).join('')}${draw.draw2.bonus !== undefined ? `<div class="divider"></div>${renderBallHTML(draw.draw2.bonus, ticketNums, true)}` : ''}</div></div>` : ''}</div></td><td class="ticket-data-column">${userTicket ? `<div class="match-count-stack"><div>${renderMatchCountHTML(draw.draw1.numbers, draw.draw1.bonus, ticketNums)}</div>${hasDraw2 ? `<div>${renderMatchCountHTML(draw.draw2.numbers, draw.draw2.bonus, ticketNums)}</div>` : ''}</div>` : '<span class="match-text">n/a</span>'}</td><td class="ticket-data-column">${winnings}</td></tr>`;
    }).join('');
    updateTicketDataColumns();
}
